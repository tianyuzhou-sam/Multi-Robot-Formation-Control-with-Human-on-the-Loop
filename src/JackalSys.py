...
class JackalSys:
    ...
    def __init__(self, configDict: dict, buildFlag=True):
        ...
        #     x_dot = f(x,u) [vcos(theta), vsin(theta), w]
        #     u = [u1; u2] = [v; w]


        # continuous-time dynamical function in casadi SX
        _contDyn = ca.SX.zeros(3, 1)
        _contDyn[0, 0] = inputs[0]*np.cos(states[2])
        _contDyn[1, 0] = inputs[0]*np.sin(states[2])
        _contDyn[2, 0] = inputs[1]
